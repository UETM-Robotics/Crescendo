...
public class SwerveModule {
    ...
    public SwerveModule(int ThrottleMotorPort, int AngleMotorPort, int angleEncoderPort, double angleOffset) {
        ...
        //this.ThrottleMotor.getEncoder().setPosition(0);
        this.ThrottleMotor.getEncoder().setPositionConversionFactor(1);
        this.ThrottleMotor.getEncoder().setVelocityConversionFactor(1);
        this.ThrottleMotor.setIdleMode(IdleMode.kBrake);
    ...
    public SwerveModule(int ThrottleMotorPort, int AngleMotorPort, int angleEncoderPort, double angleOffset, boolean reversed) {
        
        this.ThrottleMotor = new SparkMaxU(ThrottleMotorPort, MotorType.kBrushless);
        this.ThrottleMotor.getEncoder().setPositionConversionFactor(1);
        this.ThrottleMotor.getEncoder().setVelocityConversionFactor(1);

        this.ThrottleMotor.setInverted(reversed);
    ...
    */
    public void setState(SwerveModuleStateU state, boolean velocityControl) {
        ...
        turnSpeed = temp;
        err = this.angleController.getPositionError();
    }
    ...
    */
    public SwerveModuleStateU getState() {
        ...
        return new SwerveModuleStateU(this.ThrottleMotor.getEncoder().getVelocity() * RobotConstants.moduleRPM_to_MetersPerSecond,
                                    ...
                                    this.ThrottleMotor.getPosition() * Math.PI * RobotConstants.motorToSwerveDriveRatio);
    }
    ...
    */
    public SwerveModuleStateU getState(ModuleUnits throttleVelocityUnits, ModuleUnits angleUnits) {
        ...
        return new SwerveModuleStateU(velocity, Rotation2d.fromDegrees(angle), this.ThrottleMotor.getPosition() * Math.PI * RobotConstants.motorToSwerveDriveRatio);
    }    
